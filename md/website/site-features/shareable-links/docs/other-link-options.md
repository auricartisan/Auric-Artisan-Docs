...
description: Add ?lang=, ?color= or #split= to an address to open a page in a language, inspect a colour, or restore a split view.
...
- **Parameter names:** `color`, `colour` or `hex` for the colour; `on`, `bg`, `against` or `vs` for a background to compare it with.
...
- The inspector reports contrast against the page it opens on unless you give a background with `on=`.

The inspector itself is part of the colour tools; see [Basic colour tools](../../../tools/colour-workspace/basic-colour-tools/README.md).
