---
title: Auric Icons: Playful — Related extensions and comparison
description: How Auric Icons: Playful compares with Modern, Drawn, Icon Studio, Icon Hub, File Icons, Emoji and the Icon Pack.
product: VS Code extensions › Auric Icons: Playful
updated: 2026-09-25
---

# Related extensions and comparison

## At a glance

| Extension | Main job | Explorer file icon theme | Artwork | Network |
| --- | --- | --- | --- | --- |
| [Auric Icons: Playful](../README.md) | Emoji-style icon library. | No | 1,225 icons | None |
| [Auric Icons: Modern](../../auric-icons-modern/README.md) | Modern icon library plus the Modern file and folder icon theme. | Yes: **Auric Icons: Modern** | 720 library icons; 408 theme icons | None |
| [Auric Icons: Drawn](../../auric-icons-drawn/README.md) | Warm Drawn icon library. | No | 480 icons | None |
| [Icon Studio](../../auric-artisan-icon-studio/README.md) | One offline studio for every Auric collection, emoji and seven open-source sets. | No | 17,234 entries in 11 collections | None |
| [Icon Hub](../../auric-artisan-icon-hub/README.md) | Seven open-source sets with colour and stroke controls, sprite and ZIP export, and CDN insertion. | No | 12,895 icons | Optional metadata checks; CDN output loads from the web |
| [File Icons](../../auric-artisan-file-icons/README.md) | Explorer file and folder icons in three styles, plus File Icon Studio. | Yes: Facet, Playful and Modern | 1,464 SVG assets | None |
| [Emoji](../../auric-artisan-emoji/README.md) | Emoji Studio for Unicode emoji. | No | 1,914 emoji | None |
| [Icon Pack](../../auric-artisan-icon-pack/README.md) | Installs Modern, Playful, Drawn and Emoji together. | Through Modern | None of its own | Needs the Marketplace to install |

## Two different "Playful"s

- **Auric Icons: Playful** (this extension): 1,225 emoji-style icons for inserting into code.
- **Playful style in Auric Artisan File Icons**: 500 SVG file and folder icons for the Explorer.

They share a name but not artwork.

## How the three collections merge

- Install Playful with Modern and/or Drawn and one extension hosts a single merged library, with a **Library** filter, collection badges and every collection's variants. See [Use Playful with Modern and Drawn](../docs/merged-icon-studio.md).
- Install Auric Artisan Icon Studio and, by default, it hosts all three instead.
- Likes and recent icons stay with the extension that owns each icon.

## Other related pages

- [Install and set up](../../install-and-setup/README.md)
- [Icon Library on the website](../../../website/tools/colour-workspace/icon-library/README.md)
